...
- Current organisms: 17 *Prochlorococcus* + 7 *Synechococcus/Parasynechococcus* + 1 *Thermosynechococcus* (BP1) + 9 *Alteromonas* (incl. **AltDE**, **AltDE1**) + 4 heterotrophs (Shewanella, Pseudomonas, Ruegeria, Meiothermus) + 2 reference proteome match organisms (**Marinobacter (MarRef v6)**, **Alteromonas (MarRef v6)**); 45 OrganismTaxon nodes total (38 genome strains + 5 treatment organisms + 2 reference proteome match); primary focus is MED4 (NCBI taxid 59919). Authoritative per-genus breakdown lives in `tests/kg_validity/test_organism.py`.
