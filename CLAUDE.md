...
- **InterPro reference cache:** `cache/data/interpro/interpro_reference.json` (built by step 9; `{IPR: {name, type, parent, level}}`, ~54K entries; raw `entry.list` + `ParentChildTreeFile.txt` under `cache/data/interpro/raw/` are gitignored)
