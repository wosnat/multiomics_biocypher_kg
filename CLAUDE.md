...
- Experiment nodes: 76 nodes with properties: `name`, `organism_name`, `treatment_type`, `treatment`, `control`, `experimental_context`, `coculture_partner`, `omics_type`, `statistical_test`, `is_time_course`, `medium`, `temperature`, `light_condition`, `light_intensity`, `table_scope`, `table_scope_detail`. Node IDs: `{doi}_{experiment_key}`. Each groups the analyses for one scientific comparison within a publication. `coculture_partner` is only present on coculture experiments (null/absent on non-coculture). `table_scope` describes what genes the source DE table contains: `all_detected_genes` | `significant_any_timepoint` | `significant_only` | `top_n` | `filtered_subset`. `table_scope_detail` provides free-text clarification for ambiguous cases.
