...
- **InterPro reference cache:** `cache/data/interpro/interpro_reference.json` (built by step 9; `{IPR: {name, type, parent, level[, go_terms][, pathways]}}`, ~54K entries, ~9.4 MB; `go_terms`/`pathways` are sparse keys — use `.get()`. Raw `entry.list` + `ParentChildTreeFile.txt` + `interpro2go` + `interpro.xml.gz` under `cache/data/interpro/raw/` are gitignored)
