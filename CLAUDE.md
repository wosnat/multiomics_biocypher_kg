...
- **metabolism_adapter.py** — Reaction + Metabolite nodes + Gene_catalyzes_reaction / Reaction_has_metabolite / Reaction_in_kegg_pathway edges (reads `cache/data/kegg/kegg_metabolism_xrefs.json` built by prepare-data step 6)
...
**Step 6** (`multiomics_kg/download/build_kegg_metabolism_xrefs.py`) — Phase 1.2 KEGG-native metabolism layer. Walks every strain's `gene_annotations_merged.json` to identify gene-reachable KEGG R-numbers, expands to C-numbers via `/link/compound/reaction`, enriches each with KEGG metadata (name, EC, pathways) + MNX cross-refs (MNXR/MNXM, ChEBI, HMDB, InChIKey, formula, mass), and writes `cache/data/kegg/kegg_metabolism_xrefs.json` (~1 MB). The 2.6 GB MNX resolver opens here only — build-time adapter reads only the JSON cache. Run as module: `uv run python -m multiomics_kg.download.build_kegg_metabolism_xrefs [--force]`. Requires step 5.
...
- Metabolite nodes (~1.5K) with primary ID `kegg.compound:C*` (with chebi: / mnx: fallback for compounds without KEGG entry). Properties: kegg_compound_id, name, formula, mass, inchikey, smiles, mnxm_id (nullable), chebi_id (nullable), hmdb_id (nullable). Computed post-import: gene_count, organism_count.
- `Gene_catalyzes_reaction` edges from gene["kegg_reactions"]; `Reaction_has_metabolite` edges from KEGG /link/compound/reaction (direction-agnostic — Spec 1.2 explicitly does NOT model reaction direction); `Reaction_in_kegg_pathway` edges reuse existing KeggTerm pathway nodes (kegg.pathway:ko*); `Organism_has_metabolite` edges materialized post-import as a 2-hop save (Gene→Reaction→Metabolite, distinct).
