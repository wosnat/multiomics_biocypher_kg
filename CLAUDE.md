...
- **tcdb_adapter.py** — TCDB transport ontology + substrate→Metabolite bridge. Reads `cache/data/tcdb/{tcdb_hierarchy,tcdb_pruned}.json` built by step 6. Pruned to gene-annotated TC IDs **+ their ancestors only** (no downward walk)
...
11. Writes `cache/data/tcdb/tcdb_pruned.json` (`kept_tcdb_ids` + `subtree_substrates` + `seed_aliases`) and `cache/data/kegg/kegg_data.json` (~3-4 MB, indented JSON for git-friendly diffs).
