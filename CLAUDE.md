...
- **interpro_adapter.py** — InterProScan InterPro-entry ontology (hierarchical, scored, functional; `InterproEntry` nodes + `Gene_has_interpro_entry` edges (coords/evalue/score/libraries) + `Interpro_entry_is_a_interpro_entry` hierarchy + `Pfam_in_interpro_entry` bridge). Reads per-strain `<strain>.interproscan.calls.json` for edge evidence + `cache/data/interpro/interpro_reference.json` (prepare_data step 9) for node names/types/hierarchy; Pfam node set injected for a dangling-proof bridge. See `docs/kg-changes/interproscan-extension.md`
...
- `/interproscan-run` — run InterProScan 5 (Docker) per strain over `protein.faa` → per-protein domain calls across all member DBs (Pfam, NCBIfam, PROSITE, SFLD, PANTHER, Gene3D, …) integrated into InterPro entries + GO/pathway. Phase 1 produces inspectable `<strain>.interproscan.calls.json`; **Phase 2 (KG integration) is DONE** — `InterproEntry` ontology via `interpro_adapter` (see `docs/kg-changes/interproscan-extension.md`). Needs one-time `--prepare-image` + `--refresh-data` (image `interpro/interproscan:5.78-109.0` + ~6.4 G data under `~/tools/InterProScan/`). Phase-1 design: `docs/superpowers/specs/2026-07-22-interproscan-domains-design.md`
...
**Step 9** (`multiomics_kg/download/build_interpro_reference.py`) — Builds the committed InterPro reference cache. Downloads four InterPro `current_release` files — `entry.list` + `ParentChildTreeFile.txt` (names/types/hierarchy), `interpro2go` (~3 MB) and `interpro.xml.gz` (~42 MB, streamed gzip-wise, never fully decompressed) — and writes `cache/data/interpro/interpro_reference.json` (`{IPR: {name, type, parent, level[, go_terms][, pathways]}}`, ~54K entries, ~9.4 MB), consumed by `interpro_adapter` for `InterproEntry` node names/types + the is-a hierarchy. Global reference download, independent of steps 0–8. Run as module: `uv run python -m multiomics_kg.download.build_interpro_reference [--force] [--refetch-raw] [--pathway-dbs METACYC]` (`--refetch-raw` re-pulls the FTP files, only on an InterPro release). Pure parse in `multiomics_kg/utils/interpro_reference.py`.
...
- **This is entry-level and complements — does not duplicate — the per-strain `entry_xrefs.json`.** The scan artifacts carry GO/pathways only for entries some protein actually matched. The reference carries them for **every** InterPro entry, including is-a ancestors that no gene hits directly, and is the **only** source of entry-level **EC** (5,100 of 12,994 observed entries) and **CAZy** (113) — InterProScan emits GO and pathway xrefs but neither of those.
...
- **InterPro reference cache:** `cache/data/interpro/interpro_reference.json` (built by step 9; `{IPR: {name, type, parent, level[, go_terms][, pathways][, ec_numbers][, cazy_ids]}}`, ~54K entries; all four xref keys are sparse — use `.get()`. `ec_numbers` on 10,849 entries, `cazy_ids` on 227 (parsed from `interpro.xml.gz` `db="EC"`/`db="CAZY"`, stored raw — consumer normalizes). Lazily loaded inside gene-annotation build (step 2), Pfam-precedent. Raw files under `cache/data/interpro/raw/` are gitignored)
...
- InterproEntry nodes (InterProScan 5): ~12,999 nodes (InterPro entries observed on any strain + their is-a ancestors; pruned like TCDB/BRITE). Node IDs use the colon form `interpro:IPR*` (`interpro` **is** a registered bioregistry prefix). Types: FAMILY 6,490 · DOMAIN 4,355 · HOMOLOGOUS_SUPERFAMILY 1,533 · CONSERVED_SITE 390 · ACTIVE_SITE 95 · REPEAT 74 · BINDING_SITE 55 · PTM 7. Properties: name, interpro_id, interpro_type, level (is-a depth in ParentChildTree, 0=parentless root; **sparse** — ~86% are level 0). Computed post-import: gene_count (DIRECT — genes with an edge to this exact entry, the correct ORA count), organism_count, member_count (direct child entries), is_promiscuous (bool, gene_count ≥ 1000 — flags 22 ubiquitous entries like P-loop NTPase). The edge `Gene_has_interpro_entry` (Gene → node; ~397K edges; 102,895 genes ~85% covered) is a **scored** ontology edge — carries `start`/`end` (domain envelope), `evalue` (float, nullable — best/min; **omitted** when no match reports one, ~42% of matches carry none), `score` (float, nullable — best/max), `libraries` (str[], member DBs), `match_count` (int). **NO e-value cutoff** — each member DB pre-applies its curated threshold; evalue/score are evidence-only, never a filter. Hierarchy edge `Interpro_entry_is_a_interpro_entry` (child → parent; ~1,569; is-a only, 100% within-type). Bridge edge `Pfam_in_interpro_entry` (Pfam → node; ~5,236) links the eggNOG Pfam layer to InterPro (overlap link, **not** a merge). **Functional** (folded into `Gene.annotation_types` += 'interpro' + `Gene.interpro_entry_count`) but deliberately NOT into `informative_annotation_types` / `annotation_quality` (redundant with pfam/go/ec — deferred). **ORA over InterPro must stratify by `(interpro_type, level)`** — `interpro_type` PRIMARY (breadth), `level` secondary (the inverse of every other ontology; hierarchy too sparse for level to partition). Merged via the `interproscan` source (`interpro_entries`, join `protein_id`); adapter reads the calls.json directly for edge evidence. Built with `/integrate-a-tool` (Phase 2). See `docs/kg-changes/interproscan-extension.md`.
- **InterPro two-layer integration (2026-08-10)** — InterPro's entry-level xrefs now enrich genes AND link ontologies. See `docs/kg-changes/interpro-two-layer.md`.
  ...
  - InterPro ships **no KEGG** pathway xrefs (MetaCyc + species-collapsed Reactome only) — KEGG pathways stay KO-derived. GO from InterPro is entry-level (`entry_xrefs.json` / reference), reproducing per-protein scan GO 100/100. **MCP not yet updated** — source/evidence filters + 2-hop router mode are a follow-up (spec §7).
...
- Gene `informative_annotation_types: str[]` (post-import): granular per-source list parallel to legacy `annotation_types` but with the term-informativeness filter applied. 13 sources: `go_bp`, `go_mf`, `go_cc`, `pfam`, `cog_category`, `kegg`, `brite`, `ec`, `cyanorak_role`, `tigr_role`, `reaction`, `transporter`, `cazy`. The last three are exclusive to `informative_annotation_types` (not in legacy `annotation_types`). Empty when `annotation_state = 'no_evidence'`.
...
- **Source bucket maintenance**: the `annotation_quality` / `annotation_state` source-bucket list is **explicitly enumerated**, not auto-discovered. The 8 live buckets (as of 2026-05-02): `go`, `kegg`, `pfam`, `ec`, `role`, `reaction`, `transporter`, `cazy`. To add a new bucket, follow the procedure in `docs/superpowers/specs/2026-05-01-explorer-frictions-resolution-design.md` — touches the post-import Cypher (both `.cypher` and `.sh`), this CLAUDE.md, and the bucket-count test. The 13-source `informative_annotation_types` is independently maintained but coarsens to the same 8 buckets (`go = go_bp ∪ go_mf ∪ go_cc`, `role = cog_category ∪ cyanorak_role ∪ tigr_role`, `brite` extra).
