...
4. Prunes the TCDB hierarchy to the gene-reachable subhierarchy, then resolves TCDB substrate strings (`CHEBI:NNNN;name`) via the MNX resolver into Metabolite primary IDs. Gene-annotated TCIDs not present in the curated hierarchy (typically retired eggNOG IDs) are remapped to the nearest curated ancestor by truncating dot-segments and recorded in `tcdb_pruned.json`'s `seed_aliases` — the TCDB adapter consumes that map at edge-build time so no annotation is dropped.
