...
MIT9313 GCF GFF `old_locus_tag` sometimes omits the PMT0### (Cyanorak) form, causing 360 genes to split into separate Cyanorak-only and NCBI-only rows in gene_mapping.csv. A `_position_fallback_merge()` function was added that matches unmatched Cyanorak/NCBI entries by genomic position — originally by coordinate overlap (≥90% reciprocal, ≤10 bp boundaries; merged 105 genes), since 2026-08-29 by **shared stop codon / reading frame with per-contig offsets** (merges 157 on MIT9313, 923 corpus-wide, translation-verified 0 false — see `docs/methods_position_fallback_merge.md`). Also added preservation of the consumed Cyanorak locus_tag in `old_locus_tags` so it remains discoverable by gene_id_mapping.
