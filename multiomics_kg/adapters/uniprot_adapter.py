...
UNIPROT_EDGE_MODELS = [
    ...
    UniprotEdgeModel(
        uniprot_field=UniprotNodeField.xref_refseq,
...
class Uniprot:
    ...
    def __init__(
        ...
        gene_mapping_paths: Optional[list[str]] = None,
    ):
        """
        Args:
            ...
            gene_mapping_paths: Optional list of paths to gene_mapping.csv files
                (created by CyanorakNcbi adapter). Required for GENE_TO_PROTEIN
                edges — the CyanorakNcbi adapter must run first to generate these
                files. Each CSV must have 'protein_id' and 'locus_tag' columns.
                The RefSeq protein_id is used to join UniProt proteins to gene nodes.
        """
        model = UniProtModel(
            organism=organism,
            rev=rev,
        ...
        # Gene mapping for RefSeq-based gene-to-protein edges
        self.gene_mapping_paths = gene_mapping_paths or []
        self._refseq_to_genes = None  # lazy-built by _build_refseq_to_gene_map()
    ...
    def _build_refseq_to_gene_map(self) -> dict[str, list[str]]:
        """Build a mapping from RefSeq protein_id to locus_tag(s) using
        gene_mapping.csv files generated by the CyanorakNcbi adapter.

        Returns:
            Dict mapping RefSeq protein accession (e.g. 'WP_011131639.1')
            to a list of locus_tags. Multiple locus_tags occur when the same
            WP_ protein appears in multiple assemblies (e.g. Alteromonas strains).
        """
        if self._refseq_to_genes is not None:
            return self._refseq_to_genes

        refseq_map: dict[str, list[str]] = {}
        for path in self.gene_mapping_paths:
            if not os.path.exists(path):
                logger.warning(f"Gene mapping file not found: {path}")
                continue
            with open(path, 'r') as f:
                ...
                for row in reader:
                    ...
                    if protein_id and locus_tag:
                        refseq_map.setdefault(protein_id, []).append(locus_tag)

        self._refseq_to_genes = refseq_map
        logger.info(
            f"Built RefSeq-to-gene map: {len(refseq_map)} protein IDs "
            f"from {len(self.gene_mapping_paths)} mapping file(s)"
        )
        return refseq_map

    def _create_gene_to_protein_edges(
        self, edge_model: UniprotEdgeModel, properties: dict
    ) -> list[tuple]:
        """Create gene-to-protein edges by joining UniProt xref_refseq with
        gene_mapping.csv protein_id fields.

        Each UniProt protein's RefSeq accession(s) are looked up in the
        refseq-to-gene map built from gene_mapping.csv files. One edge is
        created per matching locus_tag (a protein shared across strains
        produces multiple edges).
        """
        refseq_map = self._build_refseq_to_gene_map()
        if not refseq_map:
            logger.warning(
                "No RefSeq-to-gene mapping available; skipping GENE_TO_PROTEIN edges. "
                "Ensure CyanorakNcbi adapter runs before UniProt adapter."
            )
            return []

        edge_list = []
        refseq_field = UniprotNodeField.xref_refseq.value

        if refseq_field not in self.data:
            logger.warning(
                f"Field '{refseq_field}' not in UniProt data; "
                "add UniprotNodeField.xref_refseq to node_fields to enable "
                "GENE_TO_PROTEIN edges."
            )
            return []

        matched = 0
        unmatched = 0
        for protein, refseq_val in tqdm(
            self.data[refseq_field].items(),
            desc="Getting GENE_TO_PROTEIN edges",
        ):
            if not refseq_val:
                continue

            refseq_ids = self._ensure_iterable(refseq_val)
            protein_id = self.add_prefix_to_id("uniprot", protein)
            ...
            for refseq_id in refseq_ids:
                if not refseq_id:
                    continue
                locus_tags = refseq_map.get(str(refseq_id).strip())
                if not locus_tags:
                    continue
                found = True
                for locus_tag in locus_tags:
                    gene_id = self.add_prefix_to_id("ncbigene", locus_tag)
                    edge_list.append((
                        None,
                        protein_id,
                        gene_id,
        ...
        logger.info(
            f"GENE_TO_PROTEIN edges: {len(edge_list)} edges from "
            f"{matched} matched proteins ({unmatched} unmatched)"
        )
        return edge_list
    ...
    def _create_edges_of_type(self, edge_model: UniprotEdgeModel):
        ...
        if (edge_model.edge_type in self.edge_types) and (edge_model.uniprot_field.value in self.data.keys()):
            # Special handling for GENE_TO_PROTEIN: join via RefSeq protein_id
            if edge_model.edge_type == UniprotEdgeType.GENE_TO_PROTEIN:
                return self._create_gene_to_protein_edges(edge_model, properties)
    ...
    def __init__(self, config_list_file: str, **kwargs):
        ...
        Args:
            config_list_file: Path to a CSV file with columns:
                ...
                The data_dir column is used to locate gene_mapping.csv files
                for gene-to-protein edges.

            NOTE: For GENE_TO_PROTEIN edges, the CyanorakNcbi adapter must run
            before UniProt edge generation so that gene_mapping.csv files exist
            in each data_dir.
        ...
        for organism_id, assembly_list in taxid_to_assemblies.items():
            ...
            # Build gene_mapping.csv paths from data_dirs for this taxid
            gene_mapping_paths = [
                os.path.join(d, "gene_mapping.csv")
                for d in taxid_to_data_dirs.get(organism_id, [])
            ]
            adapter = Uniprot(
