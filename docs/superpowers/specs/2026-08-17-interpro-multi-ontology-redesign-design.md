...
| `NcbifamFamily` | **new** | Observed-only (~4,957 nodes: 2,204 TIGR\*, 2,753 NF\*), flat. ID **underscore form** `ncbifam_TIGR01234` / `ncbifam_NF002735` — resolved: `ncbifam` is NOT a bioregistry prefix (`normalize_prefix` → None in the project venv), so `normalize_curie` takes the underscore fallback, psortb/signalp precedent. *(Superseded 2026-08-19, KG-SYNC-002: shipped ids are now the colon form `ncbifam:TIGR01234` / `ncbifam:NF002735` — house-minted prefix, see `docs/kg-changes/interpro-multi-ontology.md`.)* Properties: `name` (product name), `ncbifam_id`, `family_type`, `gene_symbol` (sparse), `description` (sparse — curated `comment` from the reference, 64% of observed), `level` (always 0). TIGRFAM accessions (`TIGR*`) are absorbed unmodified — enables the future `Ncbifam_family_has_tigr_role` bridge (§8) without migration |
