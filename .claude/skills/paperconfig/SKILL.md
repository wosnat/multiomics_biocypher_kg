...
| `uniprot_defline` | UniProt FASTA-style defline; build pipeline auto-extracts each `(sp\|tr)\|<acc>\|<entry_name>` triple, emitting the accession as Tier 2 `uniprot_accession` and the entry name as Tier 1 `uniprot_entry_name`. Use on columns where rows are raw UniProt deflines (sometimes concatenated with `;`). May also appear in `product_columns` for description harvest. | `sp\|Q31L36\|RF1_SYNE7` |
